(() => {
  ...
  const originalInitializeYouTube = window.initializeYouTube;
  if (typeof originalInitializeYouTube === 'function') {
    window.initializeYouTube = async function initializeYouTubeOnDemand(playlistId) {
      try {
  ...
  const originalStartPayment = window.startPayment;
  if (typeof originalStartPayment === 'function') {
    window.startPayment = async function startPaymentOnDemand(provider, course) {
