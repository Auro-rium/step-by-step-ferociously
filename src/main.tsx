...
type PaymentProvider = 'stripe' | 'razorpay' | 'crypto' | string;
...
function Checkout() {
  ...
  const pay = async (provider: 'stripe' | 'razorpay') => {
    ...
    try {
      ...
      if (provider === 'stripe') { location.assign(result.data.checkout_url); return; }
...
function Learn() {
  ...
  const canOpenLesson = (lessonIndex: number) => lessonIndex === 0 || completed.has(videoIds[lessonIndex - 1] || '');
  const selectLesson = (lessonIndex: number) => { if (!canOpenLesson(lessonIndex)) return; setIndex(lessonIndex); setWatched(0); setActiveQuiz(null); setQuizResult(null); player.current?.playVideoAt(lessonIndex); };
